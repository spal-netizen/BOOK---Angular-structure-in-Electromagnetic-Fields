...
$fresh = Join-Path $root "latex\_sec2101_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Compactness of radiation operators}"

$endMarker = "\subsection{Non-radiating source configurations}"
...
Write-Host "sec2101 replaced OK ($startBody..$endBody)"
