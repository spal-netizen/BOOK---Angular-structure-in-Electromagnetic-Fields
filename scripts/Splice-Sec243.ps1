...
$fresh = Join-Path $root "latex\_sec243_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Uniqueness of radiative solutions}"

$endMarker = "\subsection{Implications for source reconstruction}"
...
Write-Host "sec243 replaced OK ($startBody..$endBody)"
