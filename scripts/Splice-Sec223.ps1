...
$fresh = Join-Path $root "latex\_sec223_fresh.tex"
$text = [System.IO.File]::ReadAllText($ch02)
$freshBody = [System.IO.File]::ReadAllText($fresh)
$startMarker = "\subsection{Causality and time-frequency equivalence}"
$endMarker = "\subsection{Propagating, evanescent, and non-propagating subspaces}"
...
Write-Host "sec223 replaced OK ($startBody..$endBody)"
