...
$fresh = Join-Path $root "latex\_sec293_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Near-to-far transformation}"

$endMarker = "\subsection{Far-field angular accessibility}"
...
Write-Host "sec293 replaced OK ($startBody..$endBody)"
