...
$fresh = Join-Path $root "latex\_sec231_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Scalar, vector, and dyadic Green functions}"

$endMarker = "\subsection{Singularities, distributions, and regularization}"
...
Write-Host "sec231 replaced OK ($startBody..$endBody)"
