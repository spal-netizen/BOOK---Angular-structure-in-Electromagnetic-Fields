...
$fresh = Join-Path $root "latex\_sec292_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Retained, suppressed, and filtered components}"

$endMarker = "\subsection{Near-to-far transformation}"
...
Write-Host "sec292 replaced OK ($startBody..$endBody)"
