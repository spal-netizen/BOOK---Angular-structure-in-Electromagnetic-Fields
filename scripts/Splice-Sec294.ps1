...
$fresh = Join-Path $root "latex\_sec294_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Far-field angular accessibility}"

$endMarker = "\section{Examples}"
...
Write-Host "sec294 replaced OK ($startBody..$endBody)"
