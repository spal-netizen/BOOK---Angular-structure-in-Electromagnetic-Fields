...
$fresh = Join-Path $root "latex\_sec241_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Sommerfeld radiation condition}"

$endMarker = "\subsection{Outgoing-wave selection}"
...
Write-Host "sec241 replaced OK ($startBody..$endBody)"
