...
$fresh = Join-Path $root "latex\_sec222_fresh.tex"
$text = [System.IO.File]::ReadAllText($ch02)
$freshBody = [System.IO.File]::ReadAllText($fresh)
$startMarker = "\subsection{Frequency-domain Maxwell operators}"
$endMarker = "\subsection{Causality and time-frequency equivalence}"
...
Write-Host "sec222 replaced OK ($startBody..$endBody)"
