...
$fresh = Join-Path $root "latex\_sec263_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Fredholm structure and solvability}"

$endMarker = "\subsection{Geometric constraints on radiative spectra}"
...
Write-Host "sec263 replaced OK ($startBody..$endBody)"
