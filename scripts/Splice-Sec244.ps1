...
$fresh = Join-Path $root "latex\_sec244_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Implications for source reconstruction}"

$endMarker = "\section{Radiation Integrals and Source-to-Field Mappings}"
...
Write-Host "sec244 replaced OK ($startBody..$endBody)"
