...
$fresh = Join-Path $root "latex\_sec274_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Consequences for angular spectra}"

$endMarker = "\section{Reciprocity and Radiative Coupling}"
...
Write-Host "sec274 replaced OK ($startBody..$endBody)"
