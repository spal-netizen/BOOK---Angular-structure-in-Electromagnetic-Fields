...
$fresh = Join-Path $root "latex\_sec272_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Non-radiating current configurations}"

$endMarker = "\subsection{Evanescent subspaces and non-reconstructability}"
...
Write-Host "sec272 replaced OK ($startBody..$endBody)"
