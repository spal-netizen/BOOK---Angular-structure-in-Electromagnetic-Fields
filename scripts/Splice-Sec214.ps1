...
$fresh = Join-Path $root "latex\_sec214_fresh.tex"
$text = [System.IO.File]::ReadAllText($ch02)
$freshBody = [System.IO.File]::ReadAllText($fresh)
$startMarker = "\subsection{Self-adjointness, closures, and admissibility}"
$endMarker = "\section{Time- and Frequency-Domain Maxwell Operators}"
...
Write-Host "sec214 replaced OK ($startBody..$endBody)"
