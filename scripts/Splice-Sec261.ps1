...
$fresh = Join-Path $root "latex\_sec261_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Eigenfunctions and spectral decomposition}"

$endMarker = "\subsection{Rotational symmetry and spectral degeneracy}"
...
Write-Host "sec261 replaced OK ($startBody..$endBody)"
