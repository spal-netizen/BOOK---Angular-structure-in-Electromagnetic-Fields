...
$fresh = Join-Path $root "latex\_sec233_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Retardation and causal propagation}"

$endMarker = "\subsection{Inversion and solvability}"
...
Write-Host "sec233 replaced OK ($startBody..$endBody)"
