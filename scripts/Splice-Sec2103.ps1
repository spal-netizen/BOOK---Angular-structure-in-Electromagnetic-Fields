...
$fresh = Join-Path $root "latex\_sec2103_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Evanescent suppression in the far field}"

$endMarker = "\subsection{Radiation from structured source states}"
...
Write-Host "sec2103 replaced OK ($startBody..$endBody)"
