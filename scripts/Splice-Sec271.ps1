...
$fresh = Join-Path $root "latex\_sec271_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Null spaces of radiation operators}"

$endMarker = "\subsection{Non-radiating current configurations}"
...
Write-Host "sec271 replaced OK ($startBody..$endBody)"
