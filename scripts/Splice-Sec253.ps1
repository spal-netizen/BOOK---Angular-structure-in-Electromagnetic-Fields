...
$fresh = Join-Path $root "latex\_sec253_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Energy, momentum, angular-momentum flux, power, and operator norms}"

$endMarker = "\subsection{Radiative and non-radiative components}"
...
Write-Host "sec253 replaced OK ($startBody..$endBody)"
