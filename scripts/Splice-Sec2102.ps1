...
$fresh = Join-Path $root "latex\_sec2102_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Non-radiating source configurations}"

$endMarker = "\subsection{Evanescent suppression in the far field}"
...
Write-Host "sec2102 replaced OK ($startBody..$endBody)"
