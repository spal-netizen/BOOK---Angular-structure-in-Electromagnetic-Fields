...
$fresh = Join-Path $root "latex\_sec2104_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Radiation from structured source states}"

$endMarker = "\section{Chapter Summary and Forward Map}"
...
Write-Host "sec2104 replaced OK ($startBody..$endBody)"
