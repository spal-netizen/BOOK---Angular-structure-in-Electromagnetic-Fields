...
$fresh = Join-Path $root "latex\_sec254_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Radiative and non-radiative components}"

$endMarker = "\section{Spectral Theory of Radiation Operators}"
...
Write-Host "sec254 replaced OK ($startBody..$endBody)"
