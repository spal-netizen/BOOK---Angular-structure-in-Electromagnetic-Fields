...
$fresh = Join-Path $root "latex\_sec221_fresh.tex"
$text = [System.IO.File]::ReadAllText($ch02)
$freshBody = [System.IO.File]::ReadAllText($fresh)
$startMarker = "\subsection{Time-domain evolution}"
$endMarker = "\subsection{Frequency-domain Maxwell operators}"
...
Write-Host "sec221 replaced OK ($startBody..$endBody)"
