...
$fresh = Join-Path $root "latex\_sec264_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Geometric constraints on radiative spectra}"

$endMarker = "\section{Non-Radiating Sources"
...
Write-Host "sec264 replaced OK ($startBody..$endBody)"
