...
$fresh = Join-Path $root "latex\_sec213_fresh.tex"
$text = [System.IO.File]::ReadAllText($ch02)
$freshBody = [System.IO.File]::ReadAllText($fresh)
$startMarker = "\subsection{Boundedness, compactness, rank deficiency, spectral compression}"
$endMarker = "\subsection{Self-adjointness, closures, and admissibility}"
...
Write-Host "sec213 replaced OK ($startBody..$endBody)"
