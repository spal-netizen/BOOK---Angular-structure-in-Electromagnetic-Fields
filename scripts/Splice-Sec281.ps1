...
$fresh = Join-Path $root "latex\_sec281_fresh.tex"

$text = [System.IO.File]::ReadAllText($ch02)

$freshBody = [System.IO.File]::ReadAllText($fresh)

$startMarker = "\subsection{Lorentz reciprocity in operator form}"

$endMarker = "\subsection{Orthogonality and coupling of radiative modes}"
...
Write-Host "sec281 replaced OK ($startBody..$endBody)"
