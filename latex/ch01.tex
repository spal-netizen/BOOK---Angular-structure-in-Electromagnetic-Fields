...
freedom. A precise interaction map is established here at HOME level so that
...
\(\exp(-j\omega t)\) convention, let \(\J_c(\mathbf{r})\) be conduction-current
...
p_m(\mathbf{r})=\frac{1}{2}\Re\!\left\{\E\cdot\J_c^*\right\},
\end{equation}
where \((\cdot)^*\) denotes complex conjugation and \(\Re\{\cdot\}\) extracts the
real part. Operand by operand, \(\E\cdot\J_c^*\) is local field-current work-rate
...
\(\J_c\times\mu\H^*\) is magnetic-force density on moving charge carriers. Taken
...
Channel 1: \(p_m=\frac{1}{2}\Re\{\E\cdot\J_c^*\}\) integrated to \(P_m\).\\
Channel 2: \(\mathbf{f}_m=\frac{1}{2}\Re\{\rho\E+\J_c\times\mu\H^*\}\)
...
ultrafast transients require generalized models and are outside the present HOME
...
Stress-based measurement in this chapter is inherited directly from the HOME
...
Subsection~\ref{sec:ch01.1.4}: the stress tensor remains the field-theoretic HOME
object, while this subsection fixes only how that object is sampled and reported.
This HOME/USE separation prevents recursion while preserving experimental
readability.
...
channel data. The HOME projection model introduced below exposes the resolution
...
is a regime-specific evaluator obtained by applying the HOME conservation
...
subsection establishes that insufficiency at HOME level and fixes a critical
...
With a HOME counterexample now fixed, the next subsection tests an additional
...
\((\mathbf{A},\phi)\) and \((\mathbf{A}',\phi')\). The HOME gauge-variation structure already
...
reinforces HOME/USE discipline by applying, rather than restating, the gauge
...
can now close the chapter by separating established HOME results from deferred
