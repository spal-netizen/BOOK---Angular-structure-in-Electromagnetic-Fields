...
% Accessibility audit remediation 2026-05-29: Radiative Accessibility Principle,
% accessibility hierarchy, master synthesis figure, realizability foreshadowing
% Forensic remediation pass 2026-05-29: sign audit, thesis headline, prose governance
...
Radiation is not merely propagation. At fixed frequency it acts as a
\emph{structure-selective transformation}: impressed sources generate many
Maxwell-admissible field degrees of freedom, yet only a qualified outgoing
subset survives the radiation map \(\mathcal{R}_\omega=\Pi_{\mathrm{rad}}\circ\mathcal{S}_\omega\)
and reaches far-zone observables. Reactive storage, evanescent sheets, source
nullity, aperture rank limits, and geometric suppression progressively remove
\emph{inaccessible} content---a partition made explicit in
Eq.~\eqref{eq:ch2.dof-partition-master} of Section~\ref{sec:ch210}
\cite{stratton1941,harrington2001,devaney2004}. Volume~I treats that filtering as
physics, not post-processing: what fails export cannot carry the transport or
measurement claims inherited from Chapter~\ref{ch:01}.

\begin{principle}[Radiative Accessibility Principle]
\label{prin:ch2.radiative-accessibility}
Maxwell propagation does not preserve all electromagnetic structure. Radiation acts
as a structure-selective transformation that progressively suppresses electromagnetic
degrees of freedom that cannot be exported from the source region. Observable
electromagnetic structure is therefore not a complete representation of the source
manifold but rather a filtered image produced by the combined action of Maxwell
evolution, radiation selection, and measurement accessibility.
\end{principle}
...
begin. Subsection~\ref{sec:ch201} gives the first definitive account of the source-to-field
...
subsection establishes the first Volume~I account of that two-step composition:
...
\end{equation}
Equation~\eqref{eq:ch2.radiation-operator-def} is the first formal Volume~I
definition of \(\mathcal{R}_\omega\) \cite{stratton1941}.

\medskip
\noindent\fbox{\parbox{0.97\linewidth}{%
\textbf{Provisional \(\Pi_{\mathrm{rad}}\) (until Subsection~\ref{sec:ch224}).}
Until Eq.~\eqref{eq:ch2.rad-projection-composition} locks
\(\Pi_{\mathrm{rad}}=\mathcal{O}_{\mathrm{out}}\circ\Pi_{\mathrm{prop}}\), treat
\(\Pi_{\mathrm{rad}}\) as an abstract outgoing selector---not a spatial window.
Every far-zone or transport claim before Subsection~\ref{sec:ch224} is provisional
pending spectral refinement \cite{stratton1941,harrington2001}.%
}}
\medskip Transport admissibility in
...
Section~\ref{sec:ch01.1.4} and specifies which field pair enters the surface integral
when the observable is radiative. For \(\mathcal{R}_\omega[\Jimp]\), \(P_{\mathrm{rad}}(R)\) becomes
...
Chapter~\ref{ch:01}. The operator maps introduced here constrain which field content
reaches the radiative range; Chapter~\ref{ch:04} will determine which of those
radiative images admit finite-source realizations under spectral truncation and
geometry-limited accessibility.
...
Chapter~\ref{ch:02} authoritative account of this coupled system \cite{harrington2001}.
...
follow in Sections~\ref{sec:ch21}--\ref{sec:ch23}. Harmonic Maxwell closure fixes the
resolvent branch on which radiation acts; realizability of impressed patterns---whether
a target far-zone state is \(\mathcal{R}_\omega[\Jimp]\) for admissible
\(\Jimp\)---is analyzed explicitly in Chapter~\ref{ch:04} through finite geometry
and effective angular dimension.
...
The operator framework developed throughout Chapter~\ref{ch:02} may be viewed as a
preliminary \emph{Accessibility Theory of Electromagnetic Structure}. Its purpose is
to distinguish mathematically admissible electromagnetic content from physically
accessible electromagnetic content and to identify the transformations that separate
existence, radiation, and observation.
...
The provisional split \(\Pi_{\mathrm{prop}},\Pi_{\mathrm{ev}},\Pi_{\mathrm{non}}\)
earns permanent notation because no single radiation report can be posed without
separating propagating export, evanescent storage, and non-propagating sectors;
\(\Lambda_{\mathrm{nf}}\) names the near-to-far amplitude law on which angular
spectra in Section~\ref{sec:ch29} depend; and
\(\Lambda_{\mathrm{rad}},\mathcal{M}_\omega\) encode the finite-rank measurement
maps without which aperture-limited observability cannot be stated as an operator
identity \cite{stratton1941,harrington2001}.

\begin{figure}[t]
\centering
\ChOneFigBlock{%
\begin{tikzpicture}[ch1fig, node distance=7mm and 12mm]
  \node[ch1box, minimum width=42mm] (src) {Source Space\\$\mathcal{J}_\Omega$};
  \node[ch1box, below=9mm of src, minimum width=42mm] (adm) {Maxwell-Admissible\\Field Space};
  \node[ch1box, below=9mm of adm, minimum width=42mm] (rad) {Radiative Field Space\\$\mathcal{F}_{\mathrm{rad}}$};
  \node[ch1box, below=9mm of rad, minimum width=42mm] (acc) {Accessible Measurement\\Space};
  \node[ch1box, below=9mm of acc, minimum width=42mm] (obs) {Observable Space};
  \draw[ch1flow] (src) -- (adm) -- (rad) -- (acc) -- (obs);
\end{tikzpicture}%
}
\caption{Hierarchy of electromagnetic accessibility: each downward transition applies
additional physical constraints and generally reduces recoverable information.
Existence, radiation, accessibility, and observation are distinct stages rather than
equivalent descriptions of the same electromagnetic state.}
\label{fig:ch2.accessibility-hierarchy}
\end{figure}

Figure~\ref{fig:ch2.accessibility-hierarchy} restates
Principle~\ref{prin:ch2.radiative-accessibility} in pipeline form. Each transition in
the hierarchy imposes additional physical constraints and generally reduces accessible
information. Existence, radiation, accessibility, and observation therefore represent
distinct stages rather than equivalent descriptions of electromagnetic structure.
Observable quantities should be interpreted as the terminal image of this hierarchy
rather than as complete representations of the underlying source manifold.

The accessibility interpretation advanced in this subsection resolves a persistent
category error in radiation theory. Laboratory observers typically report quantities at
the terminal end of Eq.~\eqref{eq:ch2.continuity-composition-law}: a voltage, a
pattern coefficient, or a flux estimate produced after Maxwell closure, outgoing
selection, and finite instrument rank have already acted. Those readings are not
faithful portraits of the impressed source manifold \(\mathcal{J}_\Omega\); they are
compressed images whose angular content has survived radiation selection and
measurement null spaces. Existence---the full Maxwell-admissible field generated by
\(\mathcal{S}_\omega\)---therefore precedes export---the subspace reached by
\(\mathcal{R}_\omega\)---which in turn precedes observability---the finite-rank output
of \(\Lambda_{\mathrm{lab}}\) in
Eq.~\eqref{eq:ch2.operational-measurement-chain}. Chapter~\ref{ch:04} will ask which
compressed images remain realizable by finite geometry;
Principle~\ref{prin:ch1.rep-physics} already separated representation from physics in
Chapter~\ref{ch:01}; Chapter~\ref{ch:02} supplies the operator layer that makes those
distinctions computable. Radiation is structure-selective in exactly this sense: it is
not merely propagation through free space, but a progressive filter whose terminal
readings inherit only the angular organization that survived export and measurement.
...
\centering
\ChOneFigBlock{%
\begin{tikzpicture}[ch1fig, node distance=6mm and 10mm]
  \node[ch1box, minimum width=36mm] (s0) {Admissible\\Sources};
  \node[ch1box, below=7mm of s0, minimum width=36mm] (s1) {Maxwell\\Closure};
  \node[ch1box, below=7mm of s1, minimum width=36mm] (s2) {Field\\Manifold};
  \node[ch1box, below=7mm of s2, minimum width=36mm] (s3) {Radiation\\Selection};
  \node[ch1box, below=7mm of s3, minimum width=36mm] (s4) {Accessible\\Structure};
  \node[ch1box, below=7mm of s4, minimum width=36mm] (s5) {Measurement\\Operator};
  \node[ch1box, below=7mm of s5, minimum width=36mm] (s6) {Observable\\Structure};
  \draw[ch1flow] (s0) -- (s1) -- (s2) -- (s3) -- (s4) -- (s5) -- (s6);
  \node[ch1lblout, right=20mm of s2, text width=28mm, align=left] {Null Spaces\\Spectral Compression\\Finite Geometry};
  \node[ch1lblout, right=20mm of s5, text width=28mm, align=left] {Finite Apertures\\Rank Deficiency\\Source Equivalence};
\end{tikzpicture}%
}
\caption{Master synthesis pipeline for Chapter~\ref{ch:02}: impressed sources traverse
Maxwell closure and radiation selection before finite measurement maps produce
observable structure. Side labels name the principal loss mechanisms acting between
stages.}
\label{fig:ch2.master-synthesis-pipeline}
\end{figure}

Figure~\ref{fig:ch2.master-synthesis-pipeline} is the chapter-level synthesis of
Figure~\ref{fig:ch2.source-radiation-pipeline},
Figure~\ref{fig:ch2.ch1-continuity-stack}, and
Figure~\ref{fig:ch2.accessibility-hierarchy}. Every subsequent section refines one
stage or one side-label constraint without altering the downward composition order.
...
preserves them and embeds them in spaces where curl, divergence, and tangential
trace maps are closed operations. Subsection~\ref{sec:ch01.1.2}
...
functions \cite{harrington2001,stratton1941}. That workflow requires square-integrable
...
distributional structure those declarations presuppose. Finite-energy admissibility from
...
\paragraph{Closing summary.}
Sobolev field spaces Eqs.~\eqref{eq:ch2.hcurl-space}--\eqref{eq:ch2.maxwell-product-space},
...
function-analytic structure is recorded here as the definitive construction of
...
\paragraph{Closing summary.}
Operator domains and ranges Eqs.~\eqref{eq:ch2.volume-operator-domain-range}--\eqref{eq:ch2.composite-operator-domain},
...
here the constructive development establishes the operator-theoretic lemmas those
...
Finite geometry cannot export arbitrarily fine angular complexity; propagation
therefore acts as a compression mechanism on electromagnetic structure.
...
be recovered by any rank-\(N\) surrogate operator. Finite geometry cannot export
arbitrarily fine angular complexity; propagation therefore acts as a compression
mechanism on electromagnetic structure.
...
\paragraph{Closing summary.}
Coercivity on the cavity quotient Eq.~\eqref{eq:ch2.coercivity-quotient}, passive impedance
...
classes \cite{stratton1941,harrington2001}. Function-space and spectral apparatus
established here quantifies rank loss and compression; Chapter~\ref{ch:04} completes
the forward map by stating which operator outputs survive finite source support and
...
Chapter~\ref{ch:02} account of time-domain evolution on the same admissible classes
...
supplies the definitive account of \emph{time-domain} Maxwell evolution: the
...
\paragraph{Closing summary.}
The time-domain state Eq.~\eqref{eq:ch2.time-state-vector}, evolution system
...
\paragraph{Closing summary.}
Harmonic residual packaging Eq.~\eqref{eq:ch2.harmonic-maxwell-residual}, electric and magnetic
...
operators. Subsection~\ref{sec:ch223} supplies the definitive proof that
...
\eqref{eq:ch2.harmonic-equivalence-theorem} is the authoritative account of
...
\paragraph{Closing summary.}
Causal operator support Eqs.~\eqref{eq:ch2.causal-operator-support}--\eqref{eq:ch2.retarded-kernel-support},
...
and causality. Subsection~\ref{sec:ch224} supplies the definitive for
...
\medskip
\noindent\textbf{Effective definition (locked from Subsection~\ref{sec:ch224} onward).}
...
\paragraph{Closing summary.}
Dispersion split Eqs.~\eqref{eq:ch2.dispersion-relation}--\eqref{eq:ch2.longitudinal-wavenumber-split},
...
Subsection~\ref{sec:ch231} owns the constructive development of scalar, vector, and
...
account for principal-value and finite-part prescriptions, for extraction of singular
...
Chapter~\ref{ch:02} authoritative definitions for scalar, vector, and dyadic Green functions
...
Chapter~\ref{ch:02} authoritative realization of the preview
...
\paragraph{Closing summary.}
Scalar Helmholtz kernels Eqs.~\eqref{eq:ch2.scalar-helmholtz-green-pde}--\eqref{eq:ch2.free-space-scalar-green},
...
Chapter~\ref{ch:02} authoritative prescription for on-surface field evaluation; it differs
...
\paragraph{Closing summary.}
Singular-kernel expansion Eq.~\eqref{eq:ch2.dyadic-singular-expansion}, static limit
...
supplies the definitive account of \emph{retarded} time-domain Green operators,
...
\paragraph{Closing summary.}
Retarded scalar wave operator Eq.~\eqref{eq:ch2.retarded-scalar-wave-operator}, causal scalar kernel
...
supplies the definitive account of constructive inversion and solvability on
...
\eqref{eq:ch2.multi-region-inversion-system} is the constructive development of
...
Equation~\eqref{eq:ch2.discrete-green-equivalence} restates the same inversion problem
in matrix form. Ill-conditioning near
...
\paragraph{Closing summary.}
Composite-domain decomposition Eq.~\eqref{eq:ch2.composite-domain-decomposition}, patch inversion
...
Section~\ref{sec:ch263} \cite{stratton1941,harrington2001,jackson1999}. Green-function
inversion recovers field states from sources under declared closure; whether a desired
angular pattern is source-realizable remains a Chapter~\ref{ch:04} question tying
spectral truncation to finite impressed currents.
...
Exterior Maxwell problems admit multiple square-integrable branches until an outgoing selector is fixed. On a bounded domain with passive
...
Subsection~\ref{sec:ch241} supplies the definitive account of the Sommerfeld
...
\paragraph{Closing summary.}
Scalar Sommerfeld asymptotic Eq.~\eqref{eq:ch2.sommerfeld-scalar-asymptotic}, vector
...
Chapter~\ref{ch:02} account of the \emph{outgoing-wave selector}
...
\paragraph{Closing summary.}
Outgoing selector Eqs.~\eqref{eq:ch2.outgoing-selector-def}--\eqref{eq:ch2.outgoing-idempotency},
...
Chapter~\ref{ch:02} authoritative proof that exterior radiative solutions are unique on the
...
definitive uniqueness statement \cite{stratton1941,harrington2001}.
\end{theorem}
\begin{proof}
...
\paragraph{Closing summary.}
Exterior radiative uniqueness Theorem~\ref{thm:ch2.exterior-radiative-uniqueness} follows from
...
records the inverse-source consequences for \emph{inverse} source problems:
...
excite---are deferred to Chapter~\ref{ch:04}.
...
evaluations on \(\mathcal{R}_\omega[\Jimp]\) without re-opening their definitive proofs in
...
Once outgoing closure is fixed, antenna engineers need field and pattern values at prescribed observation points, not merely existence of a solution. Green convolution
...
Subsection~\ref{sec:ch251} supplies the definitive account of magnetic couplings,
...
Subsection~\ref{sec:ch251} owns the constructive development of source-to-field integral
...
states the Chapter~\ref{ch:02} account of field magnitude scalings and for the observation
...
Subsection~\ref{sec:ch251} supplies the definitive account of the full family of
...
are the Subsection~\ref{sec:ch251} specialization of
...
the Subsection~\ref{sec:ch251} surface form of the same outgoing propagation operator;
...
\paragraph{Closing summary.}
Outgoing electric volume integral Eqs.~\eqref{eq:ch2.outgoing-electric-volume-integral}--\eqref{eq:ch2.outgoing-magnetic-from-curl-integral},
...
Subsection~\ref{sec:ch252} supplies the definitive account of near, intermediate, and
...
\eqref{eq:ch2.far-field-integral-reduction} is the Subsection~\ref{sec:ch252} definitive account of
...
\paragraph{Closing summary.}
Distance-regime definitions Eq.~\eqref{eq:ch2.distance-regime-definitions}, near-zone scaling
...
are presupposed; their definitive proofs remain in Chapter~\ref{ch:01} \cite{stratton1941}. The
...
\paragraph{Closing summary.}
Flux meters are evaluated operator-theoretically through
...
\paragraph{Closing summary.}
The radiative--non-radiative operator split proceeds via
...
Radiation from finite sources decomposes into exporting modes only after outgoing selection and compactness enter jointly. On bounded
...
Chapter~\ref{ch:02} authoritative eigenvalue law
\begin{equation}
...
Subsection~\ref{sec:ch261} supplies the definitive account of eigenfunctions and
...
cokernel characterization are developed in Subsection~\ref{sec:ch263}, not repeated from
...
Subsection~\ref{sec:ch261} supplies the definitive account of eigenpairs, modal
...
\lambda_n\,c_n[\mathfrak{D}]\,\mathbf{u}_n,
\end{equation}
the Subsection~\ref{sec:ch261} restatement of compression operator
...
Hilbert space completion and measure normalization are developed in Chapter~\ref{ch:03}
...
\eqref{eq:ch2.spectral-far-pattern-expansion} is the Subsection~\ref{sec:ch261} upgrade
...
\paragraph{Closing summary.}
Radiation eigenpairs Eq.~\eqref{eq:ch2.radiation-eigenpair-def}, modal projectors
...
construction of \(\mathcal{H}_\ell\) is developed in Chapter~\ref{ch:03}
\cite{harrington2001}.
\end{proof}
...
Finite measurement arrays---sparse far-field samples, partial spherical scans, or
port voltages---recover only a finite number of radiation degrees of freedom from impressed
currents \cite{harrington2001,stratton1941}. That \emph{identifiability budget} is not an
instrument detail: it is encoded in the Fredholm index and solvability of
\(\mathcal{R}_\omega[\Jimp]=\mathbf{g}\) and finite-rank observations
\(\mathcal{O}_{\mathrm{meas}}\mathcal{R}_\omega[\Jimp]=\mathbf{b}\). Subsections
~\ref{sec:ch261}--\ref{sec:ch262} fixed radiation eigenmodes, spectral expansions, and
rotational degeneracy on \(\mathcal{F}_{\mathrm{rad}}\); Subsection~\ref{sec:ch263}
develops Fredholm structure, index, and solvability of radiation-related inversions,
importing
...
\cite{harrington2001,jackson1999}. Finite geometry cannot export arbitrarily fine angular
complexity; propagation therefore acts as a compression mechanism on electromagnetic
structure. Proposition~\ref{prop:ch2.far-pattern-from-modes} expresses
...
\paragraph{Closing summary.}
Fredholm decomposition Eq.~\eqref{eq:ch2.radiation-fredholm-decomposition}, index
...
supplies the definitive account of geometric constraints---support, boundary class,
...
\eqref{eq:ch2.geometric-symmetry-operator-def} is the Subsection~\ref{sec:ch264} constructive account of
...
\paragraph{Closing summary.}
Geometric symmetry operator Eq.~\eqref{eq:ch2.geometric-symmetry-operator-def}, support condition
...
\(\mathcal{S}_\omega[\Jimp]\) \cite{stratton1941,harrington2001}. Spectral
decomposition organizes which radiative modes propagate and which geometric sectors
remain accessible; Chapter~\ref{ch:04} determines which eigenstructures finite
sources can actually realize after truncation.

\section{Non-Radiating Sources and Null Spaces}
\label{sec:ch27}

Non-radiating sources demonstrate a fundamental distinction between electromagnetic
existence and electromagnetic observability. The absence of radiated signatures does
not imply the absence of electromagnetic structure. Instead, it identifies source
configurations whose field content remains inaccessible to far-zone observation.
...
\paragraph{Nullity, accessibility, and realizability.}
Three deficits are often merged in inverse problems and structured-beam reporting
...
\emph{Accessibility} is field-side: which directions in the angular spectrum survive
geometric, transport, and regime gates even when \(\mathcal{R}_\omega[\Jimp]\neq\mathbf{0}\)
\cite{harrington2001}. \emph{Realizability}---whether a target pattern is
\(\mathcal{R}_\omega[\Jimp]\) for some admissible \(\Jimp\)---is not decided by accessibility
alone; Chapter~\ref{ch:04} owns realizability proofs while Section~\ref{sec:ch27} supplies the
forward null-space taxonomy that constrains every claim \cite{colton1992,stratton1941}.
Subsection~\ref{sec:ch274} develops the triad in full; the present section builds the operator
null-space machinery it presupposes.
...
primary target for operator-theoretic characterization, dimensionality, and pairing with cokernel
...
Subsection~\ref{sec:ch271} supplies the definitive account of structural properties of
...
\paragraph{Radiation null space (locked definition).}
...
is the Subsection~\ref{sec:ch271} upgrade of preview
...
\paragraph{Closing summary.}
Radiation null space Eq.~\eqref{eq:ch2.radiation-nullspace-def}, adjoint kernel
...
\cite{stratton1941,harrington2001}. Subsection~\ref{sec:ch272} gives the Chapter~\ref{ch:02}
 definitive account of \emph{constructive} non-radiating impressed-current configurations---admissible
...
\paragraph{Non-radiating current (locked definition).}
...
~\ref{sec:ch272} upgrade of preview Eq.~\eqref{eq:ch2.nr-current-template-preview} and
...
\paragraph{Closing summary.}
Non-radiating current definition Eq.~\eqref{eq:ch2.nr-current-def}, admissibility
...
supplies the definitive account of evanescent subspaces, their overlap with non-radiating
...
\paragraph{Evanescent subspace (locked definition).}
...
\paragraph{Closing summary.}
Evanescent subspace Eq.~\eqref{eq:ch2.evanescent-subspace-def}, source overlap
...
Chapter~\ref{ch:02} account of how null-space, evanescent, and measurement deficits propagate to
...
\paragraph{Angular spectrum under radiation nullity).}
...
Subsection~\ref{sec:ch274} upgrade of the first clause of preview
...
\paragraph{Closing summary.}
Angular null-space law Eq.~\eqref{eq:ch2.angular-spectrum-nullspace-law}, suppressed sheet
...
unqualified near-zone reactive content \cite{stratton1941,harrington2001}. Source
null spaces explain what far-zone observation cannot detect; realizability---whether
accessible angular content is generable by finite \(\Jimp\)---is decided in
Chapter~\ref{ch:04} alongside source equivalence and effective rank.
...
Exchange of impressed source and measurement probe must leave bilinear radiation observables invariant on reciprocal media. Bilinear symmetry of radiation maps under source--probe
...
Subsection~\ref{sec:ch281} supplies the definitive operator statement: for
...
harmonic orthogonality and completeness are developed in Chapter~\ref{ch:03} with USE of
...
\begin{proposition}[Dyadic reciprocity swap)]
...
\paragraph{Closing summary.}
Reciprocal medium class Eq.~\eqref{eq:ch2.reciprocal-medium-class}, volume Lorentz identity
...
\cite{stratton1941,harrington2001}. Subsection~\ref{sec:ch282} gives the Chapter~\ref{ch:02}
 definitive account of orthogonality and coupling of radiative modes on the spectral coordinates fixed in
...
\paragraph{Radiation-modal orthogonality}
...
Subsection~\ref{sec:ch282} upgrade of the second line of preview
...
\paragraph{Radiative coupling bilinear form}
...
\paragraph{Closing summary.}
Radiation-modal orthogonality Eq.~\eqref{eq:ch2.radiation-modal-orthogonality}, coupling coefficients
...
\cite{stratton1941,harrington2001}. Subsection~\ref{sec:ch283} gives the Chapter~\ref{ch:02}
 definitive account of reciprocity in radiation and reception: transmit--receive operator interchange,
...
\paragraph{Transmit--receive reciprocity}
...
\paragraph{Closing summary.}
Port operators Eq.~\eqref{eq:ch2.port-radiation-reception-operator}, transmit--receive reciprocity
...
spectral template; Section~\ref{sec:ch29} upgrades radiation-operator action on angular spectra to
 locked statements compatible with outgoing gate Eq.~\eqref{eq:ch2.outgoing-plane-wave-gate} and
far amplitude Eq.~\eqref{eq:ch2.far-field-amplitude-integral} \cite{stratton1941}. Explicit
basis change to vector harmonics is developed in Chapter~\ref{ch:03} Subsection~\ref{sec:ch381}; harmonic
normalization belongs to Subsection~\ref{sec:ch352} with USE of Subsection~\ref{sec:ch282}
\cite{harrington2001}.

Not every near-field harmonic projects to a measurable far-zone direction; the radiation operator acts as an angular filter on source spectra.
...
explicit plane-wave filters and measurement screens are developed in Subsection~\ref{sec:ch294}
...
Subsection~\ref{sec:ch291} supplies the definitive account of plane-wave expansions and
...
change law \cite{stratton1941,harrington2001,jackson1999}. Finite-rank observation
...
harmonic completeness and normalization are developed in Chapter~\ref{ch:03}; Section~\ref{sec:ch29}
...
\paragraph{Plane-wave angular spectrum}
...
\cite{stratton1941,harrington2001}. Subsection~\ref{sec:ch292} gives the Chapter~\ref{ch:02}
 definitive account of retained, suppressed, and observation-filtered angular components on the outgoing
...
\paragraph{Retained angular amplitude}
...
\paragraph{Suppressed angular amplitude}
...
\paragraph{Filtered angular amplitude}
...
\paragraph{Angular component partition}
...
preview Eq.~\eqref{eq:ch2.retained-suppressed-filter-preview} to explicit locked definitions and
...
\paragraph{Closing summary.}
Retained angular amplitude Eq.~\eqref{eq:ch2.retained-angular-amplitude-def}, suppressed angular amplitude
...
\paragraph{Near-to-far transformation}
...
\paragraph{Closing summary.}
Far-zone extractor Eq.~\eqref{eq:ch2.far-zone-extractor-def}, near-to-far transformation
...
supplies the definitive closure for which directions on \(S^{2}\) carry
...
\paragraph{Far-field accessibility criterion}
...
\paragraph{Plane-wave accessibility filter}
...
is the Subsection~\ref{sec:ch294} plane-wave screen promised in
...
\paragraph{Angular measurement screen}
...
Subsection~\ref{sec:ch294} measurement closure: observation rank
...
\paragraph{Accessible angular budget}
...
\paragraph{Closing summary.}
Far-field accessibility Eq.~\eqref{eq:ch2.far-angular-accessibility-def}, accessibility operator
...
\paragraph{Structured source state}
...
\paragraph{Structured radiation coefficient}
...
Sections~\ref{sec:ch261} and~\ref{sec:ch263}, awaiting concrete vector spherical harmonic
(VSH) eigenbasis in Chapter~\ref{ch:03} §3.4. Hansen's spherical near-field measurement
framework and Tai's vector-harmonic synthesis supply the engineering lineage for that
representation closure \cite{hansen1988,tai1994,harrington2001,stratton1941}; plane-wave operator
